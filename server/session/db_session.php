...
class DB_Session{
	...
	public function checkIfSessionExist($session_id){
		try{
			...
			$sql = "SELECT COUNT(session_id) FROM cacani_session WHERE session_id = :session_id";
			$stmt = $db->prepare($sql);
	...
	public function getSessionTime($session_id){
		try{
			$db = new PDO(DB_STRING, DB_USER, DB_PASSWORD);
			$sql = "SELECT session_time FROM cacani_session WHERE session_id = :session_id";
			$stmt = $db->prepare($sql);
			$stmt->execute(array(':session_id' => $session_id));
			$response = $stmt->fetch();
			$db = null;
	...
	public function createSessionInDB($userName, $remote_addr, $remote_host, $remote_port, $remote_user, $user_agent, $session_id){
		try{
			$db = new PDO(DB_STRING, DB_USER, DB_PASSWORD);
			$sql = "INSERT INTO cacani_session(session_id, user_name, remote_host, remote_port, remote_user, remote_addr, user_agent, session_date, session_time) values (:session_id, :user_name, :remote_host, :remote_port, :remote_user, :remote_addr, :user_agent, NOW(), :session_time)";
